...
public:
	...
	void infer();
	void postprocess(std::vector<Object>& objs, std::vector<Object>& total_objs);
...
void YOLOv8::postprocess(std::vector<Object>& objs, std::vector<Object>& total_objs)
{
	objs.clear();
	...
	for (int i = 0; i < num_dets[0]; i++)
	{
		...
		printf("fire %d - (%.1f, %.1f, %.1f, %1.f) conf: %.3f\n", i + 1, x0, y0, x1, y1, obj.prob);
		total_objs.push_back(obj);
	}
}
...
)
{
	...
	for (auto& obj : objs)
	{
		...
		std::string conf_txt = std::to_string(std::round(obj.prob * 100) / 100);
		conf_txt = conf_txt.substr(0, 4);

		std::string text = CLASS_NAMES[obj.label] + " " + conf_txt;
